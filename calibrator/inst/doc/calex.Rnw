...
\title{Creating a sample usage for {\tt BACCO} from scratch: a cookbook}
...
code run set, and {\tt D2.int}, the observation points.
...
generic (so we don't need to write one).  The best way to do this %'
is to proceed by analogy and modify function {\tt phi.fun.toy()},
line by line.  Most of the function is straightforward to create.
...
The difficulty is that the basis functions are nonlinear.  Recall that
function {\tt h1.int()} includes a product:

<<>>=
h1.int
@ 

We can use the fact that $E(XY)=E(X)\cdot E(Y)$ if~$X$ and~$Y$ are
independent random variables.  As the prior variance matrix (viz,
{\tt jj.psi1.apriori}) is diagonal, different terms are independent.
BUT, $E(X^2)\neq\left(E(X)\right)^2$.
Thus we may copy from function {\tt E.theta.toy()} and modify as
necesary:
...
And remembering that the covariance is zero this reduces to
\[
\left(
\begin{array}{ccccc}
0&0&0&0&0\\
