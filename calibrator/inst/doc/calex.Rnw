...
for the field observations, and {\tt d.int} for both together
\end{itemize}


Objects (data and functions) in the package follow a consistent
nomenclature: anything that ends in ``{\tt .toy}'' is specific to the
toy datasets, and a replacement needs to be supplied by the user (in
this vignette, the ending ``{\tt .int}'' is used).  Anything that does
not end with ``{\tt .toy}'' is generic, in the sense that it will work
for any application and does not need be supplied by the user, and
should not be altered.  For example, {\tt hh.fun()} is generic because
it does not end with {\tt .toy}.
...
The function is in two bits: one if {\tt give.mean} is {\tt TRUE}, and
one for it being {\tt FALSE}.  If {\tt TRUE}, it returns {\tt H1()}
using the input value of {\tt D2} and $\overline{\theta}$ (as
determined from the {\em a priori} distribution).  If {\tt FALSE}, it
needs to return the matrix described in the help file\footnote{
$E_\theta(h\cdot h^T)-E_\theta(h)\cdot E_\theta(h)^T$} which in this
case is independent of {\tt x1} and {\tt x2}.  This matrix is 6-by-6
because it is the same size as the variance matrix, and that is 6-by-6
because $h(x)=\mbox{\tt c(1,x)}$ [recall that here {\tt x} is a single
row of a dataframe like {\tt D1.toy}---actually, it uses the output
of {\tt D1.fun()}---and this has five columns].
...
So the object to return (when {\tt give.mean} is {\tt FALSE}) is
this, minus
...
Notes
\begin{itemize}
\item We use the optimized value for {\tt phi}, that is {\tt
phi.stage2} (not the true value).
\item I have set {\tt eps} to 0.8, as {\tt adapt()} is very slow
...
the type given by equation 8 of the supplent ({\tt p.eqn8.supp()} in
the package: only a likelihood is given.

See the manpage for {\tt MH.Rd} for examples of the
