...
of {\tt x1} and {\tt x2}.  This matrix is 6-by-6 because it is the
same size as the variance matrix, and that is 6-by-6 because
$h(x)=\mbox{\tt c(1,x)}$ [recall that here {\tt x} is a single row of
  a dataframe like {\tt D1.toy}---actually, it uses the output of {\tt
    D1.fun()}---and this has five columns].
